...
import { ArrowRight, Trash2, Plus, Truck } from 'lucide-react';
import { fetchData, insertData, deleteData } from '../lib/dataService'; 
import { Toast } from '../components/Toast';
import { ConfirmDialog } from '../components/ConfirmDialog';

export const WholesalersScreen = ({ onBack, currentUser }) => {
  ...
  const loadWholesalers = async () => {
    const data = await fetchData('wholesalers');
    if (data) {
      // PRIVACY FILTER: Only show wholesalers for this user
      const userItems = currentUser 
        ? data.filter(c => c.user_id == currentUser.id)
        : data.filter(c => !c.user_id);
      setWholesalers(userItems);
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    const { data, error, isOffline } = await insertData('wholesalers', payload);

    if (error) {
      alert('حدث خطأ أثناء الإضافة');
    } else {
      showToast(isOffline ? 'تم الحفظ (وضع عدم الاتصال)' : 'تمت إضافة التاجر بنجاح');
      if (data) {
         loadWholesalers();
      }
      setInputValue('');
  ...
  const confirmDelete = async () => {
    ...
    const { error, isOffline } = await deleteData('wholesalers', deleteDialog.id);
    if (!error) {
      setWholesalers(wholesalers.filter(c => c.id !== deleteDialog.id));
      showToast(isOffline ? 'تم الحذف (وضع عدم الاتصال)' : 'تم حذف التاجر بنجاح');
    }
  ...
  return (
    <div className="h-screen flex flex-col bg-[#FFF9C4] overflow-hidden relative font-sans">
      ...
      <ConfirmDialog 
        ...
        title="حذف التاجر"
        message="هل أنت متأكد من حذف هذا التاجر؟"
      ...
      <div className="bg-[#00695c] text-white h-16 flex items-center px-4 shadow-lg shrink-0 rounded-b-2xl z-10">
        ...
        <h1 className="text-xl font-bold flex-1 text-center ml-10">تجار الجملة</h1>
      </div>
      ...
      <div className="flex-1 flex flex-col p-4 overflow-hidden">
        <form onSubmit={handleAdd} className="mb-6 shrink-0 flex flex-col gap-3">
          <div>
            <label className="block text-[#00695c] text-xs font-bold mb-1 text-right px-1">اسم تاجر الجملة</label>
            <input
              type="text"
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              placeholder="اكتب اسم التاجر هنا..."
            ...
            {wholesalers.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 flex-1 justify-end">
                    ...
                    <div className="w-10 h-10 rounded-lg bg-cyan-50 flex items-center justify-center text-[#0097a7]">
                        <Truck size={20} />
                    </div>
                </div>
              </div>
